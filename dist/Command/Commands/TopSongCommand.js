...
module.exports = class TopSongCommand extends Command_1.default {
    constructor() {
        super("topsong", {
            category: "beatsaber",
    ...
    async sendScoreMessage(user, channel, guildSettings, offsetArg) {
        const topSongs = await super.instance.beatSaberManager.fetchScores(user.scoreSaberId, "TOP", 0);
        const player = await super.instance.beatSaberManager.getPlayer(user.scoreSaberId);
        if (topSongs == null || player == null) {
            await channel.send("Unable to fetch top song...");
            return;
        }
        let song = topSongs[0];
        if (offsetArg) {
